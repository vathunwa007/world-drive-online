...
export const Map3D: React.FC<Map3DProps> = ({ apiKey, mapId, myCar, myCarType, myCarColor, playerName, peersRef, onMapError }) => {
  ...
  useEffect(() => {
    const handleWheel = (e: WheelEvent) => {
      ...
      zoomRef.current = Math.max(10, Math.min(22, zoomRef.current));
    };
    ...
    Promise.all([
      ...
      mapInstance.addListener('renderingtype_changed', () => {
        if (mapInstance.getRenderingType() === 'RASTER') {
          if (onMapError) {
            onMapError('The map is not a vector map. WebGLOverlayView requires a vector map. Please check your Map ID.');
          }
        }
      });
      ...
      overlay.onDraw = ({ gl, transformer }) => {
        ...
        if (myCarMeshRef.current && myCarMeshRef.current.userData.frontWheels) {
          ...
          myCarMeshRef.current.userData.frontWheels.forEach((wheelGroup: THREE.Group) => {
        ...
        cameraRef.current.projectionMatrix.fromArray(
          transformer.fromLatLngAltitude(
            ...
            new Float32Array([myCar.pitch || 0, myCar.roll || 0, -myCar.heading]),
        ...
        peersRef.current.forEach((peer, id) => {
          ...
          if (!mesh || mesh.userData.carType !== peer.carType || mesh.userData.carColor !== peer.carColor) {
            ...
            mesh = createCarMesh(peer.carType, peer.carColor, peer.playerName, false);
          ...
          cameraRef.current!.projectionMatrix.fromArray(
            transformer.fromLatLngAltitude(
              ...
              new Float32Array([peer.pitch || 0, peer.roll || 0, -peer.heading]),
              new Float32Array([1, 1, 1])
            )
          );
...
function createCarMesh(type: string, colorHex: string, playerName?: string, isMe: boolean = false) {
  ...
  gltfLoader.load(
          ...
          if (mesh.material) {
            if (Array.isArray(mesh.material)) {
              mesh.material.forEach(mat => {
                if ('color' in mat) (mat as THREE.MeshStandardMaterial).color.copy(color);
              });
            } else {
              if ('color' in mesh.material) (mesh.material as THREE.MeshStandardMaterial).color.copy(color);
            }
          }
        }
      });
    ...
    undefined,
    (error) => {
      console.warn(`Failed to load model ${modelUrl}, using fallback geometry.`, error);
    }
  );
...
function createFallbackCar(group: THREE.Group, type: string, color: THREE.Color) {
  // Base dimensions
  let width = 2;
  let height = 1;
  let length = 4;
